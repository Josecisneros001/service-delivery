...
class Login extends Component<{is_service_provider: boolean},LoginState,{}> {
  ...
  async handleSubmit(event: React.ChangeEvent<any>) {
    ...
    const response = await Users.login(this.state.email, this.state.password, this.props.is_service_provider ? 1 : 0);
    if(response.status !== 200) {
      this.setState({snackBarMsg: 'Email or Password incorrect, please try again.', showAlert: true});
      return;
    }
    this.setState({isAuth: true});
    handleLogInCookies(response.data.id, response.data.token, this.props.is_service_provider);
  }
    ...
    return (
      <>
        ...
        <div className="flex flex-col w- 3/5 md:w-2/5 m-auto bg-gray-50 mt-16 shadow-lg text-center">
          ...
          <form onSubmit={this.handleSubmit} className="flex-1 flex flex-col text-2xl" autoComplete="nope">
          ...
          <hr />
          <div className="items-center py-2">
            <button className="button button2" onClick={()=>this.setState({redirectToCreate: true})}>
              Create new account
            </button>
          </div>
        </div>
